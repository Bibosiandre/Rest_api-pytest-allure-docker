...
class TestUserAuth(BaseCase):
    ...
    @allure.description("Test authorize success by email and password")
    def test_auth_user(self):
        ...
        Assertions.assert_json_value_by_name(
            response2, "user_id",
            self.user_id_from_auth_method, "User id is not equal"
        )
    @allure.description("Test check success w/o sending auth cookie or token")
    ...
    def test_negative_auth_check(self, condition):
        ...
        Assertions.assert_json_value_by_name(
            response2, "user_id",
           0, f"User is authorized with condition {condition}"
        )
